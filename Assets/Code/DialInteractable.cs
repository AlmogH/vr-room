...
public class DialInteractable : XRBaseInteractable
{
    ...
    // public FloatVariable forceValue;
    public StringVariable forceValueStr;
        ...
        if (isSelected)
        {
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Fixed)
            {
                ...
                if (DialType == InteractionType.ControllerRotation)
                {
                    ...
                    // forceValue.SetValue(Vector3.SignedAngle(m_GrabbedRotation.eulerAngles, m_SyncTransform.eulerAngles, worldRotationAxis));
                    if (angle < 0) angle = 360 + angle;
                    Debug.Log(forceValueStr);
                    if (forceValueStr != null)
                    {
                        forceValueStr.Value = worldRotationAxis.ToString();
                    }
    ...
    protected override void OnSelectExited(SelectExitEventArgs args)
    {
        ...
        // Debug.Log("Interactor:" + Vector3.Angle(args.interactor.transform.forward, Vector3.up));
        // Debug.Log("Interactable:" + Vector3.Angle(args.interactable.transform.forward, Vector3.up));
